...
//        label.setText("Hello World!");
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        addSelectListener(tableView1);
        addSelectListener(tableView2);
        tableView1.setEditable(true);
    }
    
    @FXML
    private void saveTable1AsPlaylist(ActionEvent event) {
        saveListAsPlayList(new ArrayList<>(tableView1.getItems()));
    }

    @FXML
    private void saveTable1SelectionAsPlaylist(ActionEvent event) {
        List<MediaItem> selection = new ArrayList<>(tableView1.getSelectionModel().getSelectedItems());
        saveListAsPlayList(selection);
    }
    
    @FXML
    private void saveTable2AsPlaylist(ActionEvent event) {
        saveListAsPlayList(new ArrayList<>(tableView2.getItems()));
    }

    @FXML
    private void saveTable2SelectionAsPlaylist(ActionEvent event) {
        List<MediaItem> selection = new ArrayList<>(tableView2.getSelectionModel().getSelectedItems());
        saveListAsPlayList(selection);
    }
    
    /**/
    
    private void saveListAsPlayList(List<MediaItem> mediaItems){
        FileChooser fc = new FileChooser();
        fc.setTitle("save as Playlist");
        fc.getExtensionFilters().add(new ExtensionFilter("M3U", "*.m3u"));
        File newFile = fc.showSaveDialog(rootNode.getScene().getWindow());

        if (newFile != null) {
            List<String> fileLines
                    = new PlayListServiceM3U()
                            .getPlayList(mediaItems, true);
